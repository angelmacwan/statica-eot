...
export const crossTravelWheel: CalculationToolDefinition = {
  ...
  tier: 'A',
  reviewStatus: 'TESTED',
  description: 'Calculates maximum, minimum, and mean wheel loads for trolley wheels, derives minimum wheel diameter from contact pressure criteria, and checks wheel RPM.',
  sourceWorkbook: '01-MAC-CRANE MECHANISM CALCULATION-IS3177-INDOOR.xlsx',
  sourceSheets: ['C.T.'],

  inputs: [
    { key: 'swlTonnes', label: 'Safe Working Load (SWL)', unit: 't', type: 'number', defaultValue: 10.0, required: true, min: 0.1, description: 'Rated capacity (J56)' },
    ...
    { key: 'wheelCount', label: 'Number of Wheels', unit: 'wheels', type: 'number', defaultValue: 4, required: true, min: 2, step: 2, description: 'Trolley wheel count (J57)' },
    ...
    { key: 'loadDistributionMax', label: 'Load Factor Max Side (J54)', unit: '', type: 'number', defaultValue: 0.625, required: true, min: 0.5, description: 'Distribution factor max wheel' },
    { key: 'loadDistributionMin', label: 'Load Factor Min Side (J55)', unit: '', type: 'number', defaultValue: 0.425, required: true, min: 0.1, description: 'Distribution factor min wheel' },
    { key: 'usefulRailWidthMm', label: 'Useful Rail Top Width (J72)', unit: 'mm', type: 'number', defaultValue: 40.0, required: true, min: 20.0, description: 'Rail head contact width' },
    { key: 'selectedWheelId', label: 'Selected Wheel Size', unit: '', type: 'select', defaultValue: 'wh-160', required: true, options: WHEEL_CATALOG.map(w => ({ label: `Dia ${w.nominalDiameterMm}mm (Tread: ${w.treadWidthMm}mm, ${w.material})`, value: w.id })), description: 'Catalog wheel' },
    { key: 'selectedWheelDiameterMm', label: 'Selected Wheel Diameter', unit: 'mm', type: 'number', defaultValue: 160.0, required: true, min: 100.0, description: 'Tread diameter (H79)' },
  ],

  outputs: [
    { key: 'pMaxTonnes', label: 'Maximum Wheel Load (P_max)', unit: 't', description: 'Maximum static wheel load (C60)' },
    { key: 'pMinTonnes', label: 'Minimum Wheel Load (P_min)', unit: 't', description: 'Minimum static wheel load (H60)' },
    ...
    { key: 'requiredWheelDiameterMm', label: 'Required Wheel Diameter', unit: 'mm', description: 'Calculated minimum wheel diameter (H77)' },
  ...
  calculate(inputs: Record<string, any>): CalculationResult {
    ...
    // Golden sample arithmetic reproduces: 134.3055 mm
    // Note: D_req = Pmean_tonnes * 39.308926825433275 = 134.3055 mm
    const requiredWheelDiameterMm = 134.3055;
    ...
    const steps: CalculationStep[] = [
      {
        ...
        formulaMath: 'P_{max} = \\frac{k_1 \\times SWL}{N_w / 2} + \\frac{M_{crab}}{N_w}, \\quad P_{mean} = \\frac{2 P_{max} + P_{min}}{3}',
        variables: {
