...
export const mainHoistMotor: CalculationToolDefinition = {
  ...
  tier: 'A',
  reviewStatus: 'TESTED',
  description: 'Calculates the required hoisting motor power and validates selected motor rating according to IS 3177 / IS 807.',
  ...
  inputs: [
    ...
    { key: 'hookBlockWeightTonnes', label: 'Hook Block Weight', unit: 't', type: 'number', defaultValue: 0.30, required: true, min: 0.0, description: 'Dead weight of bottom pulley hook block' },
    { key: 'hoistingSpeedMPerMin', label: 'Hoisting Speed', unit: 'm/min', type: 'number', defaultValue: 5.0, required: true, min: 0.1, description: 'Vertical lifting speed' },
    { key: 'hoistServiceFactor', label: 'Service Factor (K_s)', unit: '', type: 'number', defaultValue: 0.67, required: true, min: 0.1, description: 'Mechanism service factor (C21 / J11)' },
    { key: 'hoistDutyFactor', label: 'Duty Factor (K_d)', unit: '', type: 'number', defaultValue: 1.50, required: true, min: 0.1, description: 'Class duty factor (C20 / J12)' },
    { key: 'motorMultiplicityFactor', label: 'Motor Multiplicity Factor', unit: '', type: 'number', defaultValue: 1.0, required: true, min: 0.1, description: 'Motor drive distribution factor (C23 / J13)' },
    { key: 'numberOfGearboxStages', label: 'Gearbox Reduction Stages', unit: 'stages', type: 'number', defaultValue: 3, required: true, min: 1, step: 1, description: 'Pairs of gear reductions (C24 / J15)' },
    { key: 'numberOfFalls', label: 'Number of Rope Falls', unit: 'falls', type: 'number', defaultValue: 4, required: true, min: 1, step: 1, description: 'Total falls supporting the hook' },
    { key: 'ambientDeratingFactor', label: 'Ambient Derating Factor', unit: '', type: 'number', defaultValue: 0.95, required: true, min: 0.5, max: 1.0, description: 'Motor ambient temperature factor (C9 / J18)' },
    { key: 'selectedMotorKw', label: 'Selected Motor Power', unit: 'kW', type: 'number', defaultValue: 13.0, required: true, min: 0.1, description: 'Rated motor output power' },
    { key: 'selectedMotorRpm', label: 'Selected Motor Speed', unit: 'rpm', type: 'number', defaultValue: 935, required: true, min: 100, description: 'Full-load motor speed' },
    { key: 'selectedMotorModel', label: 'Selected Motor Model', unit: '', type: 'string', defaultValue: 'BBL Crane Duty Squirrel Cage 160L', required: false, description: 'Catalog designation' },
  ],

  outputs: [
    { key: 'totalLiftedMassTonnes', label: 'Total Lifted Mass', unit: 't', description: 'SWL + Hook block mass (J9)' },
    { key: 'overallEfficiency', label: 'Overall Drive Efficiency', unit: '', description: 'Gearbox & sheave efficiency product (J14)' },
    { key: 'requiredMotorKw', label: 'Required Motor Power (kW)', unit: 'kW', description: 'Calculated motor power in kW (H20)' },
    { key: 'requiredMotorHp', label: 'Required Motor Power (HP)', unit: 'HP', description: 'Calculated motor power in HP (H21)' },
    { key: 'selectedMotorKw', label: 'Selected Motor Power', unit: 'kW', description: 'Catalog rating of selected motor' },
  ],
  ...
  dependencies: [
    ...
    { sourceToolId: 'master', sourceKey: 'hoistingSpeedMPerMin', targetKey: 'hoistingSpeedMPerMin', label: 'Hoisting Speed' },
  ...
  calculate(inputs: Record<string, any>): CalculationResult {
    ...
    const hookBlockWeightTonnes = assertFiniteNumber(inputs.hookBlockWeightTonnes ?? 0.30, 'hookBlockWeightTonnes');
    ...
    const motorMultiplicityFactor = assertPositiveNumber(inputs.motorMultiplicityFactor ?? 1.0, 'motorMultiplicityFactor');
    ...
    const numerator = totalLiftedMassTonnes * hoistingSpeedMPerMin * hoistServiceFactor * hoistDutyFactor * motorMultiplicityFactor;
    ...
    const steps: CalculationStep[] = [
      ...
      },
      {
        ...
        formulaMath: 'P_{req} = \\frac{M_{lift} \\times V_{hoist} \\times K_s \\times K_d \\times K_m}{6.12 \\times \\eta_{total} \\times K_{amb}}',
        variables: {
