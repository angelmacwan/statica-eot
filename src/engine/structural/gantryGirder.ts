...
export const gantryGirder: CalculationToolDefinition = {
  ...
  description: 'Calculates section properties, centroid, moments of inertia with explicit rounding, wheel reactions, and bending stress for heavy-duty gantry crane box girders.',
  sourceWorkbook: 'GIRDER _CALC_40T & 60T GANTRY CRANE.xlsx',
  sourceSheets: ['MG-40T', 'MG-60T'],

  inputs: [
    { key: 'swlTonnes', label: 'Crane Safe Working Load (SWL)', unit: 't', type: 'number', defaultValue: 40.0, required: true, min: 5.0, description: 'Gantry rated capacity (e.g. 40t or 60t)' },
    { key: 'spanM', label: 'Gantry Girder Span (L)', unit: 'm', type: 'number', defaultValue: 10.0, required: true, min: 4.0, description: 'Girder span center-to-center' },
    { key: 'trolleyWeightTonnes', label: 'Trolley Weight', unit: 't', type: 'number', defaultValue: 9.2, required: true, min: 1.0, description: 'Crab dead weight' },
    { key: 'flangeWidthCm', label: 'Flange Plate Width (b)', unit: 'cm', type: 'number', defaultValue: 49.0, required: true, min: 20.0, description: 'Width of top/bottom plates (E282)' },
    { key: 'girderDepthCm', label: 'Total Girder Depth (D)', unit: 'cm', type: 'number', defaultValue: 100.0, required: true, min: 40.0, description: 'Total depth of girder (E283)' },
    { key: 'topFlangeThkCm', label: 'Top Flange Thickness (t_tf)', unit: 'cm', type: 'number', defaultValue: 1.0, required: true, min: 0.5, description: 'Top plate thickness (E284)' },
    { key: 'bottomFlangeThkCm', label: 'Bottom Flange Thickness (t_bf)', unit: 'cm', type: 'number', defaultValue: 1.0, required: true, min: 0.5, description: 'Bottom plate thickness (E285)' },
    { key: 'webThkCm', label: 'Web Plate Thickness (t_w)', unit: 'cm', type: 'number', defaultValue: 0.8, required: true, min: 0.4, description: 'Thickness of two web plates (E286)' },
    { key: 'impactFactor', label: 'Impact Factor', unit: '', type: 'number', defaultValue: 1.32, required: true, min: 1.0, description: 'Dynamic vertical impact factor' },
  ],

  outputs: [
    { key: 'sectionAreaCm2', label: 'Cross Section Area (A)', unit: 'cm2', description: 'Section area (H284)' },
    { key: 'centroidFromBottomCm', label: 'Centroid from Bottom (y_b)', unit: 'cm', description: 'Neutral axis height (H285)' },
    { key: 'ixxRoundedCm4', label: 'Moment of Inertia Ixx (Rounded)', unit: 'cm4', description: 'Preserves explicit ROUND(...,0) from sheet (H286)' },
    { key: 'iyyRoundedCm4', label: 'Moment of Inertia Iyy (Rounded)', unit: 'cm4', description: 'Preserves explicit ROUND(...,0) from sheet (H287)' },
    { key: 'girderUnitWeightKgPerM', label: 'Girder Bare Unit Weight', unit: 'kg/m', description: 'Area * 0.785 (H292)' },
    { key: 'girderTotalWeightKgPerM', label: 'Girder Weight with Diaphragms', unit: 'kg/m', description: 'Total weight including internal diaphragms (H293)' },
  ],
  ...
  calculate(inputs: Record<string, any>): CalculationResult {
    ...
    const centroidFromBottomCm = ((b * t_tf * y_tf_center) + (webDepthCm * 2 * t_w * y_web_center) + (b * t_bf * y_bf_center)) / sectionAreaCm2;
    ...
    // H287 = ROUND(2*(b^3*t_tf/12) + 2*webDepth*(t_w^3/12) + 2*webDepth*t_w*((b/2 - t_w/2)^2), 0)
    const iyyRaw =
    ...
    // With diaphragms allowance: approximately + 10 kg/m
    ...
    const checks: CalculationCheck[] = [
      {
        ...
        message: 'Gantry girder calculations are classified as Tier B and require engineering review before fabrication release.',
      },
    ];
    ...
    return {
      ...
      outputs: {
        ...
        girderTotalWeightKgPerM: { value: girderTotalWeightKgPerM, unit: 'kg/m', label: 'Total Weight with Diaphragms' },
      },
      checks,
      steps,
      assumptions: [
